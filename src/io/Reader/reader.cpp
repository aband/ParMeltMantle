...
REMARKS:
  ...
  (3) Arrays are read with readArray, using the syntax:
        ...
        6. random <minValue> <maxValue>
        7. ??? (return ERR_UNSUPPORTED_OPTION for possible further processing)
******************************************************************************/
...
Description: Read a <TYPE> array up to the size of the given array. 
             ...
             5. { [<units>] <value> ... } (units apply to all numbers)
             6. random <minValue> <maxValue>
             7. ??? (return ERR_UNSUPPORTED_OPTION for possible further
                     processing)
...
int readArray(double* array, int len) {
  ...
  case 'u': {
    ...
    if(!strcmp(str,"constant")) {
      ...
      error = readScalar(minValue); if(error) return error;
      error = readScalar(maxValue); if(error) return error;
      for(int i=0; i<len; i++) array[i] = minValue + (maxValue - minValue) * rand() / RAND_MAX;
    } else return ERR_UNSUPPORTED_OPTION;
    return 0;
  }
  case BEGIN_LIST: {
...
int readArray(double* array, int len, int* dimen, double& unitVal) {
  ...
  case 'u': {
    ...
    if(!strcmp(str,"constant")) {
      ...
      double minValue, maxValue;
      error = readScalar(minValue,dimen,unitVal); if(error) return error;
      error = readScalar(maxValue,dimen,unitVal); if(error) return error;
      for(int i=0; i<len; i++) array[i] = minValue + (maxValue - minValue) * rand() / RAND_MAX;
    } else return ERR_UNSUPPORTED_OPTION;
    return 0;
  }
...
int readVector(std::vector<int>& array) {
  return readArray(array.data(), array.size());
}
...
int readVector(std::vector<double>& array, int* dimen, double& unitVal) {
  return readArray(array.data(), array.size(), dimen, unitVal);
}
